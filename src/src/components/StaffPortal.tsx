...
import { getLiveBookingsAndRooms } from '../lib/roomAvailability';
...
export default function StaffPortal({ onGoHome }: StaffPortalProps) {
  ...
  useEffect(() => {
    ...
    // Pull live bookings from Supabase cloud so staff screens stay synced across devices
    getLiveBookingsAndRooms().then(() => loadData()).catch(() => {});
    ...
    // Auto-refresh continuously every 2 seconds for local changes
