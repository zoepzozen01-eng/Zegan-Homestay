...
import { Search, Calendar, Users, FileText, CheckCircle2, Clock, XCircle, AlertTriangle, ArrowRight, Upload, Check, CreditCard, ExternalLink, Image as ImageIcon, Coffee, Minus, Plus, Send, Sparkles, MessageSquare, UtensilsCrossed, Cookie, Copy } from 'lucide-react';
              ...
              displayedList.map((booking) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* QRIS & Transfer Bank BPD DIY workflow for PENDING bookings */}
                    {isPending && (
                      <div className="bg-brand-50/50 rounded-2xl p-4 sm:p-6 border border-brand-100 space-y-5">
                        <div className="flex flex-col lg:flex-row gap-6 items-start">
                          ...
                          {/* Payment Instruction & Bank BPD DIY Box */}
                          <div className="space-y-3 text-xs flex-1 w-full">
                            <div className="border-b border-brand-200/60 pb-2">
                              <h4 className="font-bold text-brand-950 text-sm flex items-center gap-1.5">
                                <CreditCard className="w-4 h-4 text-brand-700" />
                                {lang === 'id' ? 'Instruksi Pembayaran Resmi Zegan' : 'Official Payment Instructions'}
                              </h4>
                              <p className="text-stone-600 leading-relaxed mt-1 text-[11px]">
                                {lang === 'id' 
                                  ? `Silakan scan QRIS resmi di samping atau transfer ke rekening Bank BPD DIY. Nominal tagihan sebesar Rp${booking.total_price?.toLocaleString('id-ID')} terkunci otomatis.`
                                  : `Please scan the official QRIS or transfer to Bank BPD DIY account below. The payment amount of Rp${booking.total_price?.toLocaleString('id-ID')} is pre-locked.`}
                              </p>
                            </div>

                            {/* Bank BPD DIY Details Card */}
                            <div className="bg-white p-4 rounded-xl border border-brand-200/90 shadow-2xs space-y-2.5">
                              <div className="flex justify-between items-center border-b border-stone-100 pb-2">
                                <span className="font-extrabold text-stone-900 text-xs flex items-center gap-1.5">
                                  <span>🏦</span> BANK BPD DIY
                                </span>
                                <span className="text-[10px] text-stone-500 font-medium">
                                  {qris.bankBranch || '[003] Cabang Wates'}
                                </span>
                              </div>
                              ...
                              <div className="flex justify-between items-center bg-stone-50 p-2.5 rounded-lg border border-stone-200">
                                <div>
                                  <span className="text-[9px] uppercase font-bold text-stone-400 block tracking-wider">
                                    Nomor Rekening (BPD DIY)
                                  </span>
                                  <div className="font-mono font-black text-stone-900 text-sm tracking-wider mt-0.5">
                                    003.211.005851
                                  </div>
                                </div>
                                <button
                                  ...
                                  className="px-2.5 py-1.5 bg-brand-700 hover:bg-brand-800 text-white rounded-lg text-[10px] font-bold flex items-center gap-1 cursor-pointer transition-colors shadow-2xs"
                                >
                                  {copiedNorek ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                                  <span>{copiedNorek ? (lang === 'id' ? 'Tersalin' : 'Copied') : (lang === 'id' ? 'Salin Rek' : 'Copy')}</span>
                                </button>
                              </div>

                              <div className="text-[11px] text-stone-700 bg-brand-50/60 p-2 rounded-lg border border-brand-100">
                                <span className="text-stone-500 text-[10px] block">Atas Nama Rekening:</span>
                                <span className="font-bold text-brand-950">TRIYANTO RAHARJO, S.Sos., M.Si.</span>
                              </div>
                            </div>
                          </div>
                        </div>
                                  ...
                                  <div key={item.id} className="flex items-center justify-between p-2.5 rounded-lg bg-brand-50/50 border border-brand-100 text-xs">
                                    <div className="flex items-center gap-2.5">
                                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                                        item.category === 'coffee' || item.category === 'herbal'
