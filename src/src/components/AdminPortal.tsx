...
import { 
  ...
  Download, FileText, CheckCircle2, Clock, XCircle, AlertTriangle, AlertCircle,
...
export default function AdminPortal({ lang, staffUser, initialTab, onLogout, onTabChange }: AdminPortalProps) {
  ...
  // Cloud Database Sync & Vercel Diagnostic state
  const [dbConnectionStatus, setDbConnectionStatus] = useState<'checking' | 'connected' | 'unconfigured' | 'error'>('checking');
  ...
  const fetchDbBookings = async (roomsList = dbRooms) => {
    ...
    if (!supabaseDebugInfo.isValidUrl || !supabaseDebugInfo.keyDefined) {
      setDbConnectionStatus('unconfigured');
      setDbStatusDetails(lang === 'id' 
        ? 'Variabel VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY belum diatur di Vercel.' 
        : 'VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY environment variables are not set on Vercel.');
      return;
    }

    try {
      setIsSyncing(true);
      // Attempt 1: Fetch with relational guests join
      ...
      // Attempt 2: If foreign key error or relationship not present in Supabase, fetch direct table
      if (queryResult.error) {
        console.warn('[AdminPortal] Relational query failed, attempting direct table select:', queryResult.error);
        ...
        if (!fallbackRes.error && fallbackRes.data) {
          queryResult = fallbackRes as any;
        }
      }

      if (queryResult.error) {
        setDbConnectionStatus('error');
        setDbStatusDetails(queryResult.error.message || 'Gagal terhubung ke tabel Supabase.');
        console.warn('Failed to fetch bookings from database, using local backups.', queryResult.error);
        return;
      }

      setDbConnectionStatus('connected');
      setDbStatusDetails(lang === 'id' ? 'Database Cloud Supabase Terhubung Aktif' : 'Supabase Cloud Database Connected');

      if (queryResult.data) {
        const mappedBookings: Booking[] = queryResult.data.map((b: any) => {
          ...
          return {
            ...
            room_id: b.room_id,
            room_name: roomObj ? roomObj.type : (b.room_name || 'Kamar Zegan'),
            room_number: roomObj ? roomObj.number : (b.room_number || ''),
            check_in: b.check_in,
            check_out: b.check_out,
            guests: b.adults || b.guests || 1,
            full_name: guest?.full_name || b.full_name || 'Tamu',
            email: guest?.email || b.email || '',
            phone: guest?.phone || b.phone || '',
            special_requests: b.special_request || b.special_requests || '',
            total_price: b.total_price || 0,
            status: b.booking_status || b.status || 'Pending',
            payment_status: b.payment_status || 'Pending',
            payment_proof: b.payment_proof_url || b.payment_proof || null,
            payment_proof_url: b.payment_proof_url || b.payment_proof || null,
      ...
      setDbConnectionStatus('error');
      setDbStatusDetails(err.message || 'Error koneksi database');
  ...
  useEffect(() => {
    ...
    // Auto-poll bookings from Supabase every 15 seconds to catch live bookings from guests
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8 animate-fadeIn">
          ...
          <div>
            ...
            <div className="text-xs text-stone-400 mt-2 flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold transition-all cursor-pointer border ${
                  dbConnectionStatus === 'connected'
                    ...
                    : dbConnectionStatus === 'unconfigured'
                    ? 'bg-amber-950/80 border-amber-600/70 text-amber-300 hover:bg-amber-900 animate-pulse'
                    : dbConnectionStatus === 'error'
                    ? 'bg-red-950/80 border-red-700 text-red-300 hover:bg-red-900'
                    : 'bg-stone-800 border-stone-700 text-stone-400'
                }`}
                title="Klik untuk diagnosa koneksi database & panduan Vercel"
              >
                ...
                <span>
                  {dbConnectionStatus === 'connected' && 'Cloud Database Online'}
                  {dbConnectionStatus === 'unconfigured' && '⚠️ Mode Offline (Vercel Env Belum Diisi)'}
                  {dbConnectionStatus === 'error' && '⚠️ Koneksi Database Terputus'}
                  {dbConnectionStatus === 'checking' && 'Memeriksa Database...'}
                </span>
                <span className="text-[9px] underline opacity-80 ml-0.5">Panduan &amp; Status</span>
              </button>
              ...
              <button
                ...
                disabled={isSyncing}
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-stone-800 hover:bg-stone-750 text-stone-300 text-[11px] border border-stone-700 transition-all cursor-pointer disabled:opacity-50"
                title="Tarik pemesanan terbaru dari database Supabase sekarang"
              >
      ...
      {dbConnectionStatus === 'unconfigured' && (
        <div className="bg-amber-500/10 border border-amber-500/40 rounded-2xl p-4 sm:p-5 text-amber-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm animate-fadeIn">
          <div className="flex items-start gap-3.5">
            <div className="p-2.5 bg-amber-500/20 text-amber-300 rounded-xl mt-0.5 shrink-0">
              <AlertTriangle className="w-5 h-5" />
            </div>
            <div>
              <h4 className="font-bold text-white text-sm">
                Penyebab Pesanan Belum Masuk ke Admin: Cloud Database Belum Aktif di Vercel
              </h4>
              <p className="text-xs text-amber-200/90 mt-1 leading-relaxed max-w-3xl">
                Karena variabel <code className="bg-amber-950/60 px-1 py-0.5 rounded text-amber-300 font-mono text-[11px]">VITE_SUPABASE_URL</code> dan <code className="bg-amber-950/60 px-1 py-0.5 rounded text-amber-300 font-mono text-[11px]">VITE_SUPABASE_ANON_KEY</code> belum diset di Vercel, pemesanan dari pengunjung umum saat ini hanya tersimpan di browser lokal HP pengunjung tersebut. Tambahkan variabel ini di Settings Vercel agar semua pesanan otomatis masuk ke Admin secara online.
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={() => setShowVercelGuideModal(true)}
            className="shrink-0 bg-amber-500 hover:bg-amber-400 text-stone-950 font-bold text-xs uppercase tracking-wider px-4 py-2.5 rounded-xl transition-all shadow-md cursor-pointer whitespace-nowrap"
          >
            Lihat Solusi Vercel &rarr;
          </button>
        </div>
      )}
      ...
      <div className="min-h-[50vh]">
        ...
        {activeTab === 'settings' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fadeIn">
            ...
            {/* QRIS & Bank BPD DIY Config */}
            <div className="bg-white rounded-2xl border border-brand-200 p-6 sm:p-8 space-y-6 shadow-xs">
              <div className="border-b pb-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-serif font-bold text-brand-950 text-lg">Konfigurasi Rekening &amp; QRIS Resmi</h3>
                    <p className="text-xs text-stone-500 mt-0.5">Kelola rekening Bank BPD DIY, NMID QRIS Nasional, dan instruksi bayar.</p>
                  </div>
                  <span className="text-[10px] font-mono bg-brand-50 text-brand-850 px-2.5 py-1 rounded-full font-bold border border-brand-200">
                    BPD DIY &bull; QRIS GPN
                  </span>
                </div>
              </div>
              ...
              <form onSubmit={handleSaveQris} className="space-y-4 text-xs font-semibold text-stone-700">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="block text-[11px] text-stone-500 uppercase tracking-wider">Nama Bank</label>
                    <input
                      type="text"
                      value={qrisForm.bankName}
                      onChange={(e) => setQrisForm({ ...qrisForm, bankName: e.target.value })}
                      placeholder="Bank BPD DIY"
                      className="w-full p-2.5 border rounded-lg focus:ring-1 focus:ring-brand-700 text-xs text-brand-950"
                      required
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    <label className="block text-[11px] text-stone-500 uppercase tracking-wider">Kantor Cabang</label>
                    <input
                      type="text"
                      value={qrisForm.bankBranch || '[003] Cabang Wates'}
                      onChange={(e) => setQrisForm({ ...qrisForm, bankBranch: e.target.value })}
                      placeholder="[003] Cabang Wates"
                      className="w-full p-2.5 border rounded-lg focus:ring-1 focus:ring-brand-700 text-xs text-brand-950"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="block text-[11px] text-stone-500 uppercase tracking-wider">Nomor Rekening</label>
                    <input
                      type="text"
                      value={qrisForm.accountNumber || '003.211.005851'}
                      onChange={(e) => setQrisForm({ ...qrisForm, accountNumber: e.target.value })}
                      placeholder="003.211.005851"
                      className="w-full p-2.5 border rounded-lg focus:ring-1 focus:ring-brand-700 text-xs text-brand-950 font-mono font-bold"
                      required
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="block text-[11px] text-stone-500 uppercase tracking-wider">Atas Nama Rekening</label>
                    <input
                      type="text"
                      value={qrisForm.accountName}
                      onChange={(e) => setQrisForm({ ...qrisForm, accountName: e.target.value })}
                      placeholder="TRIYANTO RAHARJO S SOS M SI"
                      className="w-full p-2.5 border rounded-lg focus:ring-1 focus:ring-brand-700 text-xs text-brand-950"
                      required
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="block text-[11px] text-stone-500 uppercase tracking-wider">NMID QRIS Nasional</label>
                    <input
                      type="text"
                      value={qrisForm.nmid || 'ID1025401239294'}
                      onChange={(e) => setQrisForm({ ...qrisForm, nmid: e.target.value })}
                      placeholder="ID1025401239294"
                      className="w-full p-2.5 border rounded-lg focus:ring-1 focus:ring-brand-700 text-xs text-brand-950 font-mono"
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="block text-[11px] text-stone-500 uppercase tracking-wider">Kode Terminal / Subcode</label>
                    <input
                      type="text"
                      value={qrisForm.subCode || 'A01'}
                      onChange={(e) => setQrisForm({ ...qrisForm, subCode: e.target.value })}
                      placeholder="A01"
                      className="w-full p-2.5 border rounded-lg focus:ring-1 focus:ring-brand-700 text-xs text-brand-950 font-mono"
                    />
                  </div>
                ...
                <div className="space-y-1">
                  <label className="block text-[11px] text-stone-500 uppercase tracking-wider">Gambar QRIS (URL / Base Payload)</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    className="w-full p-2.5 border rounded-lg focus:ring-1 focus:ring-brand-700 text-xs text-brand-950 h-16"
                    required
                  />
                </div>
                ...
                <div className="pt-2 border-t border-stone-200">
                  <label className="block text-[11px] text-stone-500 uppercase tracking-wider mb-2">
                    Pratinjau Standee QRIS Resmi (Tampilan Tamu):
                  </label>
                  <div className="flex justify-center p-3 bg-stone-100 rounded-xl border border-stone-200">
                    <OfficialQrisCard amount={350000} showAmountBadge={true} compact={true} allowZoom={true} />
                  </div>
                </div>
                ...
                >
                  Simpan Pengaturan Pembayaran &amp; Rekening
                </button>
              </form>
            </div>
      ...
      {/* MODAL: DIAGNOSA DATABASE CLOUD & PANDUAN VERCEL */}
      <AnimatePresence>
        {showVercelGuideModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto animate-fadeIn">
            <motion.div
              ...
              className="bg-stone-900 border border-stone-800 rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl text-stone-100 my-8 space-y-6"
            >
              {/* Header */}
              <div className="flex items-start justify-between gap-4 border-b border-stone-800 pb-5">
                <div className="flex items-center gap-3">
                  <div className={`p-3 rounded-2xl ${
                    dbConnectionStatus === 'connected' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'
                  }`}>
                    <Database className="w-6 h-6" />
                  </div>
                  <div>
                    <h3 className="text-xl font-bold font-serif text-white">
                      Status Database &amp; Panduan Vercel
                    </h3>
                    <p className="text-xs text-stone-400 mt-0.5">
                      Penyebab &amp; Solusi agar pesanan dari web umum masuk ke halaman Admin
                    </p>
                  </div>
                </div>
                <button
              ...
              {/* Status Box */}
              <div className="bg-stone-950/80 rounded-2xl p-5 border border-stone-800/80 space-y-3">
                <h4 className="text-xs font-bold uppercase tracking-wider text-amber-400 flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4" />
                  <span>Status Environment Variables Saat Ini</span>
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="bg-stone-900 p-3 rounded-xl border border-stone-800">
                    <span className="text-stone-400 block text-[10px] uppercase font-mono">VITE_SUPABASE_URL</span>
                    <span className={`font-bold mt-1 flex items-center gap-1.5 ${supabaseDebugInfo.urlDefined ? 'text-emerald-400' : 'text-red-400'}`}>
                      {supabaseDebugInfo.urlDefined ? '✅ Terkonfigurasi' : '❌ Belum diset (Kosong)'}
                    </span>
                  </div>
                  <div className="bg-stone-900 p-3 rounded-xl border border-stone-800">
                    <span className="text-stone-400 block text-[10px] uppercase font-mono">VITE_SUPABASE_ANON_KEY</span>
                    <span className={`font-bold mt-1 flex items-center gap-1.5 ${supabaseDebugInfo.keyDefined ? 'text-emerald-400' : 'text-red-400'}`}>
                      {supabaseDebugInfo.keyDefined ? '✅ Terkonfigurasi' : '❌ Belum diset (Kosong)'}
                    </span>
                  </div>
                </div>
                <div className="text-xs text-stone-400 flex items-center gap-2 pt-1 border-t border-stone-850">
                  <span className="text-[11px] text-stone-500">Status Aktif:</span>
                  <span className={`font-medium ${dbConnectionStatus === 'connected' ? 'text-emerald-400' : 'text-amber-400'}`}>
                    {dbStatusDetails || 'Sedang memeriksa...'}
                  </span>
                </div>
              </div>

              {/* Explanation of Why It Happens on Vercel */}
              <div className="space-y-2 text-xs leading-relaxed text-stone-300">
                <h4 className="font-bold text-white text-sm flex items-center gap-2">
                  <AlertCircle className="w-4 h-4 text-amber-400" />
                  <span>Kenapa pesanan tidak masuk saat di Vercel?</span>
                </h4>
                <p>
                  Aplikasi ini berjalan sebagai <em>client-side React</em>. Jika variabel Supabase di atas belum dimasukkan ke pengaturan Vercel, aplikasi otomatis menggunakan sistem cadangan <strong>LocalStorage</strong> di browser.
                </p>
                <p className="bg-stone-800/60 p-3 rounded-xl border border-stone-750 text-stone-300">
                  💡 <strong>LocalStorage</strong> hanya tersimpan di memori HP atau laptop milik pengunjung itu sendiri. HP pengunjung tidak bisa mengirim data ke laptop Admin tanpa perantara database internet (Supabase).
                </p>
              </div>

              {/* Step-by-Step Vercel Fix */}
              <div className="space-y-3 bg-stone-950/50 p-5 rounded-2xl border border-stone-800 text-xs">
                <h4 className="font-bold text-white text-sm">
                  Cara Menyelesaikan di Vercel (Hanya butuh 2 menit):
                </h4>
                <ol className="space-y-2.5 list-decimal list-inside text-stone-300">
                  <li className="leading-relaxed">
                    Buka <a href="https://vercel.com/dashboard" target="_blank" rel="noreferrer" className="text-amber-400 underline font-semibold">Dashboard Vercel</a> &rarr; Pilih project <strong>Zegan Homestay</strong> Anda.
                  </li>
                  <li className="leading-relaxed">
                  ...
                  <li className="leading-relaxed">
                    ...
                    <div className="mt-2 space-y-1.5 font-mono text-[11px] bg-stone-900 p-2.5 rounded-lg border border-stone-800">
                      <div><span className="text-amber-400">Key:</span> VITE_SUPABASE_URL <br /><span className="text-stone-500">Value:</span> URL Project Supabase Anda (misal: https://xyz.supabase.co)</div>
                      <div className="pt-1.5 border-t border-stone-800"><span className="text-amber-400">Key:</span> VITE_SUPABASE_ANON_KEY <br /><span className="text-stone-500">Value:</span> anon key dari Supabase Anda</div>
                    </div>
                  </li>
                  <li className="leading-relaxed">
                    <strong>PENTING (Redeploy):</strong> Buka tab <strong>Deployments</strong> di Vercel &rarr; Klik titik tiga (<strong>...</strong>) pada deployment terakhir &rarr; Pilih <strong>Redeploy</strong>. (Vite memerlukan redeploy agar environment variables dapat dimasukkan ke dalam build).
                  </li>
                </ol>
              </div>
              ...
              <div className="space-y-2 text-xs">
                <div className="flex items-center justify-between">
                  <h4 className="font-bold text-white">Tabel Database Supabase &amp; Izin Akses (RLS):</h4>
                  <button
                    type="button"
                    onClick={() => {
                      const sqlText = `-- Jalankan di Supabase Dashboard > SQL Editor > New Query
ALTER TABLE IF EXISTS public.guests ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS public.bookings ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS "Allow anon insert guests" ON public.guests FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow anon select guests" ON public.guests FOR SELECT TO anon USING (true);

CREATE POLICY IF NOT EXISTS "Allow anon insert bookings" ON public.bookings FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow anon select bookings" ON public.bookings FOR SELECT TO anon USING (true);
CREATE POLICY IF NOT EXISTS "Allow anon update bookings" ON public.bookings FOR UPDATE TO anon USING (true);`;
                      navigator.clipboard.writeText(sqlText);
                      setSqlCopied(true);
                      setTimeout(() => setSqlCopied(false), 3000);
                    }}
                    className="text-amber-400 hover:text-amber-300 font-bold flex items-center gap-1.5 cursor-pointer underline text-[11px]"
                  >
                    {sqlCopied ? '✅ Berhasil Disalin!' : '📋 Salin Script SQL Policy'}
                  </button>
                </div>
                <p className="text-stone-400 text-[11px]">
                  File skema lengkap tersedia di root project dengan nama <code className="text-amber-300 font-mono">supabase-schema.sql</code>.
                </p>
              </div>

              {/* Actions Footer */}
              <div className="flex flex-col sm:flex-row gap-3 pt-3 border-t border-stone-800">
                <button
                  type="button"
                  onClick={() => setShowVercelGuideModal(false)}
                  className="flex-1 py-3 bg-stone-800 hover:bg-stone-750 text-stone-200 font-bold rounded-xl text-xs uppercase tracking-wider transition-all cursor-pointer text-center"
                >
                  Tutup
                </button>
                <button
                  ...
                  disabled={isSyncing}
                  className="flex-1 py-3 bg-brand-700 hover:bg-brand-600 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                >
                  <RefreshCw className={`w-4 h-4 ${isSyncing ? 'animate-spin' : ''}`} />
                  <span>{isSyncing ? 'Menguji & Menyinkronkan...' : 'Tes Ulang Koneksi & Sinkronkan'}</span>
                </button>
              </div>
