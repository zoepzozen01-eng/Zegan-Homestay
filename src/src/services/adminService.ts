...
export const DEFAULT_QRIS_SETTINGS: QrisSettings = {
  ...
  accountName: 'TRIYANTO RAHARJO S SOS M SI',
  accountNumber: '003.211.005851',
  bankBranch: '[003] Cabang Wates',
  nmid: 'ID1025401239294',
  subCode: 'A01',
  printedBy: '93600112',
  printVersion: '1.0-2025.05.06',
  instructions: 'Silakan scan QRIS resmi ZEGAN HOMESTAY di atas menggunakan e-wallet atau Mobile Banking apa pun, atau transfer langsung ke rekening Bank BPD DIY Cabang Wates no. rek 003.211.005851 a.n TRIYANTO RAHARJO S SOS M SI.'
};
...
export function getQrisSettings(): QrisSettings {
  try {
    ...
    if (raw) {
      ...
      // Auto-update if it is missing BPD DIY account number or uses old bank name
      if (!parsed.accountNumber || !parsed.accountNumber.includes('005851') || parsed.bankName === 'QRIS Nasional (GPN)') {
        const merged = { 
